const Quote = {
  ...
  getPhotoPlan: (plot) => {
    ...
    // Calculate top-down shots based on actual coverage at 400ft
    let topDownTarget = 1;
    if (plot && plot.area && typeof CoverageCalculator !== 'undefined') {
      const specs = CONFIG.droneSpecs || {};
      const topDownAltitudeFeet = 400;
      const topDownAltitudeMeters = topDownAltitudeFeet * 0.3048; // ~122m
    ...
    // Cap top-down at 50% of land photos to ensure angled shots
    const maxTopDown = Math.max(1, Math.floor(packageTotal * 0.5));
    topDownTarget = Math.min(topDownTarget, maxTopDown);
